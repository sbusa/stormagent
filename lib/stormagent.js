...
(function() {
  ...
  StormAgent = (function(_super) {
    ...
    StormAgent.prototype.run = function(config, schema) {
      ...
      return this.app = require('zappajs')(this.config.port, function() {
        var errorhandler, logger, morgan;
        morgan = require('morgan');
        morgan.token('date', _agent.timestamp);
        logger = morgan(":date - :method :url :status :response-time ms - :remote-addr");
        errorhandler = require('errorhandler');
        this.use((require('body-parser')).urlencoded(), logger, require("passport").initialize(), 'static');
        this.set({
          'basepath': '/v1.0'
        });
        this.set({
