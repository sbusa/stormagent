...
(function() {
  ...
  StormRegistry = (function(_super) {
    ...
    StormRegistry.prototype.remove = function(key) {
      ...
      if ((this.db != null) && (this.entries[key] != null) && this.entries[key].saved) {
        this.db.rm(key);
      }
  ...
  StormAgent = (function(_super) {
    ...
    function StormAgent(config) {
      var extend;
      this.log = stormlog;
      this.extend = extend = require('util')._extend;
      ...
      try {
        ...
        if (storm.functions != null) {
          ...
          if (!this.state.running) {
            this.config = this.extend(this.config, pkgconfig);
          }
        ...
        return function(repeat) {
          ...
          return async.waterfall([
            function(next) {
              ...
              return srequest(request, "" + storm.tracker + "/serialkey/" + storm.skey, storm, function(err, res, body) {
                var agent, error;
                try {
                  if (err) {
                    next(err);
                ...
                try {
                  switch (res.statusCode) {
                    case 200:
                      ...
                      storm.bolt = _this.extend(storm.bolt, bolt);
                      return next(null, storm);
                    default:
                      return next(err);
                  }
