...
(function() {
  ...
  StormAgent = (function(_super) {
    ...
    StormAgent.prototype["import"] = function(id) {
      var err, p, pkgconfig, plugfile, plugin, self, _i, _len, _ref, _ref1;
      ...
      try {
        ...
        if (pkgconfig != null) {
          this.log("import - [" + id + "] found package.config");
        }
        if (pkgconfig.storm.agent) {
          this.log("import - [" + id + "] is an agent, getting config and functions...");
          this.config = extend(this.config, pkgconfig);
          delete this.config.storm;
          this.log("import - [" + id + "] available functions:", pkgconfig.storm.functions);
          if (pkgconfig.storm.functions != null) {
            (_ref = this.functions).push.apply(_ref, pkgconfig.storm.functions);
          }
        }
        if (pkgconfig.storm.plugins) {
          this.log("import - [" + id + "] available plugins:", pkgconfig.storm.plugins);
          _ref1 = pkgconfig.storm.plugins;
        ...
        this.log("import - [" + id + "] is a storm compatible module");
      } catch (_error) {
        err = _error;
